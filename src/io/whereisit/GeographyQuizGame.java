  ...
  public void reformatStatusBar() {
    ...
    if (currentCard != null) {
      ...
      if (neither.isSelected()) {
        ...
        status += "Region" + keyString + ": " + currentCard.front + ", " + currentCard.back;
      } else if (front) {
      ...
      if (dist < c.extent) {
        ...
        if (minx < x && x < maxx && miny < y && y < maxy) {
          if (!prevCountry.equals(c.name)) {
            prevCountry = c.name;
            for (Country ci: countries) {
              if (ci.name.equals(c.name) && !ci.center.equals(c.center)) {
  ...
  public void loadCards(String dataFile) throws IOException {
    ...
    while (scan.hasNextLine()) {
      ...
      if (arr.length > 2) {
        ...
        System.out.println("adding " + c.front + " " + c.back + " to the current stacks");
